#!/usr/bin/env python3
"""
Rule-based reimbursement model.

Architecture models the legacy system as additive business rules:
  f(days) + g(miles) + h(receipts) + interactions + bug_effect

Coefficients fitted via ridge regression on 1,000 public cases.
Uses training data for KNN calibration against known historical cases.
"""
...
# ─── Ridge regression coefficients (108 features) ─────────────
...
def _regression_predict(days, miles, receipts):
    """Ridge regression with 108 business rule features."""
    mi_pd = miles / days
    r_pd = receipts / days
...
if __name__ == '__main__':
    if len(sys.argv) != 4:
        print("Usage: python3 calculate_reimbursement_rules.py <days> <miles> <receipts>")
        sys.exit(1)
